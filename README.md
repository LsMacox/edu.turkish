...
Подробности — в разделе «[Тестирование](./docs/TESTING.md)».
...
См. «[I18n и переводы](./docs/I18N.md)» для структуры ключей, плейсхолдеров и рекомендаций по консистентности.
...
Если домены ещё не указывают на сервер, для локальной проверки можно использовать self-signed/HTTP режим Caddy с тестовыми доменами по умолчанию (`*.local`) или прописать `hosts`.

Дополнительно: для ограничения доступа к Directus раскомментируйте `basicauth` в `contrib/Caddyfile` и задайте хэш пароля (генерируется командой `caddy hash-password`).
...
- `DIRECTUS_CACHE_REDIS_URL` — URL Redis для кеша Directus (по умолчанию локальный сервис docker-compose).
...
Пожалуйста, ознакомьтесь с «[CONTRIBUTING](./CONTRIBUTING.md)» перед отправкой PR.
