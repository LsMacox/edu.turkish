    ...
    <div class="container mx-auto px-4 lg:px-6">
      <div class="text-center mb-10 md:mb-16">
        <h2 class="text-4xl lg:text-5xl font-bold text-secondary mb-6">
          {{ $t('reviews.shareExperience.title') }} <span class="text-primary">{{ $t('reviews.shareExperience.titleAccent') }}</span>
        </h2>
      ...
      <div class="max-w-4xl mx-auto">
        <div class="bg-gradient-to-br from-blue-50 to-purple-50 rounded-3xl p-6 md:p-8 lg:p-12 shadow-custom">
          <form @submit.prevent.stop="submitReview" class="space-y-6 md:space-y-8">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
              ...
              <div>
                ...
                <UiFormsBaseSelect v-model="form.reviewerType">
                  <option value="">{{ $t('reviews.shareExperience.form.reviewerType.placeholder') }}</option>
                  <option value="student">{{ $t('reviews.shareExperience.form.reviewerType.student') }}</option>
                  <option value="parent">{{ $t('reviews.shareExperience.form.reviewerType.parent') }}</option>
                </UiFormsBaseSelect>
              </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
              ...
              <div>
                ...
                >
                  <option value="">
                    {{ isLoadingUniversities ? $t('reviews.shareExperience.form.university.loading') : $t('reviews.shareExperience.form.university.placeholder') }}
                  </option>
                  <option 
                  ...
                  <option value="other">{{ $t('reviews.shareExperience.form.university.other') }}</option>
                </UiFormsBaseSelect>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
              ...
              <div>
                ...
                <UiFormsBaseSelect v-model="form.year">
                  <option value="">{{ $t('reviews.shareExperience.form.year.placeholder') }}</option>
                  <option value="2024">2024</option>
                  <option value="2023">2023</option>
                  <option value="2022">2022</option>
                  <option value="2021">2021</option>
                  <option value="2020">2020</option>
                  <option value="other">{{ $t('reviews.shareExperience.form.year.earlier') }}</option>
                </UiFormsBaseSelect>
              </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
              ...
              <div>
                ...
                <UiFormsBaseSelect v-model="form.rating">
                  <option value="">{{ $t('reviews.shareExperience.form.rating.placeholder') }}</option>
                  <option value="5">{{ $t('reviews.shareExperience.form.rating.excellent') }}</option>
                  <option value="4">{{ $t('reviews.shareExperience.form.rating.good') }}</option>
                  <option value="3">{{ $t('reviews.shareExperience.form.rating.satisfactory') }}</option>
                  <option value="2">{{ $t('reviews.shareExperience.form.rating.poor') }}</option>
                  <option value="1">{{ $t('reviews.shareExperience.form.rating.veryPoor') }}</option>
                </UiFormsBaseSelect>
            ...
            <div>
              ...
              <div class="flex flex-col space-y-3 md:flex-row md:space-y-0 md:space-x-6">
                <label class="flex items-center space-x-2 cursor-pointer p-2 rounded-lg hover:bg-gray-50 transition-colors">
                  ...
                  />
                  <span class="text-secondary text-sm md:text-base">{{ $t('reviews.shareExperience.form.recommend.yes') }}</span>
                </label>
                <label class="flex items-center space-x-2 cursor-pointer p-2 rounded-lg hover:bg-gray-50 transition-colors">
                  <input 
                    type="radio" 
                    ...
                    class="text-primary focus:ring-primary"
                  />
                  <span class="text-secondary text-sm md:text-base">{{ $t('reviews.shareExperience.form.recommend.maybe') }}</span>
                </label>
                <label class="flex items-center space-x-2 cursor-pointer p-2 rounded-lg hover:bg-gray-50 transition-colors">
                  <input 
                    type="radio" 
                    ...
                    class="text-primary focus:ring-primary"
                  />
                  <span class="text-secondary text-sm md:text-base">{{ $t('reviews.shareExperience.form.recommend.no') }}</span>
                </label>
              </div>
            </div>
            ...
            <div class="flex justify-center pt-4 md:pt-6">
              ...
              >
                {{ isSubmitting ? $t('reviews.shareExperience.form.submitting') : $t('reviews.shareExperience.form.submit') }}
              </button>
            </div>
          </form>
        </div>
        ...
        <!-- Success message -->
        <div v-if="submitted" class="mt-6 md:mt-8 bg-green-50 border border-green-200 rounded-xl p-4 md:p-6 text-center">
          <Icon name="mdi:check-circle" class="text-green-500 text-3xl md:text-4xl mb-3 md:mb-4" />
          <h3 class="text-lg md:text-xl font-bold text-green-800 mb-2">{{ $t('reviews.shareExperience.success.title') }}</h3>
        ...
        <!-- Error message -->
        <div v-if="submissionError" class="mt-6 md:mt-8 bg-red-50 border border-red-200 rounded-xl p-4 md:p-6 text-center">
          <Icon name="mdi:alert-circle" class="text-red-500 text-3xl md:text-4xl mb-3 md:mb-4" />
          <h3 class="text-lg md:text-xl font-bold text-red-800 mb-2">{{ $t('reviews.shareExperience.error.title') }}</h3>
...
async function submitReview() {
  console.log('[ShareExperience] submitReview clicked')
  if (!validateForm()) {
