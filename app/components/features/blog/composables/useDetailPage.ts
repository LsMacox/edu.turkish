...
export function useDetailPage<T extends DetailPageItem>(
  ...
  const heroMeta = computed(() => {
    ...
    if (data.publishedAtLabel) {
      result.push({ icon: 'mdi:calendar', label: data.publishedAtLabel })
    }

    if (data.readingTimeLabel) {
      result.push({ icon: 'mdi:clock-outline', label: data.readingTimeLabel })
    }
  ...
  const quickFacts = computed<BlogArticleQuickFact[]>(() => {
    ...
    // Published date
    if (includePublishedDate && data.publishedAtLabel) {
      addFact({
        title: t(`${i18nPrefix}.published`),
        value: data.publishedAtLabel,
        icon: 'mdi:calendar',
      })
    }

    // Reading time
    if (includeReadingTime && data.readingTimeLabel) {
      addFact({
