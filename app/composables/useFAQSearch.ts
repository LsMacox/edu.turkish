import type { FAQItem, FAQCategory, SearchState, SearchOptions } from '~/types/preferences'
...
export const useFAQSearch = (options: SearchOptions = {}) => {
  ...
  const faqData = useState<any>('faqSearch:data', () => ({ data: [], categories: [], meta: { total: 0, filtered: 0 } }))
  ...
  const fetchFAQData = async () => {
    try {
      ...
      if (!Array.isArray(faqData.value.categories) || faqData.value.categories.length === 0 || categoriesLocale.value !== locale.value) {
  ...
  const highlightSearchTerms = (text: string, query: string): string => {
    ...
    const exactRegex = new RegExp(`(${normalizedQuery.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')})`, 'gi')
    highlightedText = highlightedText.replace(exactRegex, `${highlightTags.open}$1${highlightTags.close}`)
    
    // Then highlight individual words (if not already highlighted)
    queryWords.forEach(word => {
      const wordRegex = new RegExp(`(?!<mark[^>]*>)(${word.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')})(?![^<]*</mark>)`, 'gi')
      highlightedText = highlightedText.replace(wordRegex, `${highlightTags.open}$1${highlightTags.close}`)
    })
  ...
  const updateURL = (immediate: boolean = false) => {
